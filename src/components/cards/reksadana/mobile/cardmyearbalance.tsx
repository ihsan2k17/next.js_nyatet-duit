...
import { GiPayMoney, GiMicrochip } from 'react-icons/gi'
import { LiaMicrochipSolid } from 'react-icons/lia'

interface CardMYeardBalanceProps {
...
const CardMobileYearBalance = ({sumYearPerPortfolio, loading, card, alert}:CardMYeardBalanceProps) => {
    ...
    if(loading) {
        ...
        <div className="flex flex-col gap-2 w-[70%]">
            <LoadingText text={`Loading Your Year Balance ...`}/>
        </div>
        )
    }
    if (card.length === 0) {
    ...
    return (
        <div className={`relative flex flex-1 min-w-[320px] w-full 
            ...
            <div className="flex items-center mt-2">
                <LiaMicrochipSolid className="text-3xl drop-shadow-lg" />
            </div>

            {/* Balance */}
            <div className="mb-5">
                <p className="text-xs font-semibold opacity-80">Total Balance Year</p>
...
export default CardMobileYearBalance
